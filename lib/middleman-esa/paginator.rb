  ...
  module Esa
    ...
    class Paginator
      ...
      def initialize(app, esa_controller)
        ...
        @per_page = esa_controller.options.per_page
        @page_link = esa_controller.options.page_link
      end
      ...
      def manipulate_resource_list(resources)
        ...
        resources.each do |res|
          ...
          # Skip other esas' resources
          next unless match_esa(res, md)

          # "articles" local variable is populated by Calendar and Tag page generators
          # If it's not set then use the complete list of articles
          # TODO: Some way to allow the frontmatter to specify the article filter?
          articles = md[:locals]["articles"] || @esa_controller.data.articles

          # Allow esa.per_page and esa.page_link to be overridden in the frontmatter
      ...
      # Does this resource match the esa controller for this paginator?
      # @return [Boolean]
      def match_esa(res, md)
        res_controller = md[:locals]["esa_controller"] || (res.respond_to?(:esa_controller) && res.esa_controller)
        return false if res_controller && res_controller != @esa_controller
        override_controller = md[:page]["esa"]
        return false if override_controller && override_controller.to_s != @esa_controller.name.to_s
      ...
      def page_locals(page_num, num_pages, per_page, prev_page_res, articles)
...

        {
          ...
          # of "esa.articles" for consistency with the calendar and tag templates.
          'articles' => articles,
          'esa_controller' => @esa_controller
        }
      end
